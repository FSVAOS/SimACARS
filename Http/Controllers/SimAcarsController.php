...
namespace Modules\SimACARS\Http\Controllers;
...
use Modules\SimACARS\Entities\VamEvent;
use Modules\SimACARS\Entities\VamTrack;

class SimAcarsController extends Controller
{
    ...
    public function getAircraft(Request $request)
    {
        ...
        $allFleets = Aircraft::orderBy('name')->get();
        //Debugger::log($allFleets);

        foreach ($allFleets as $fleet) {
            $aircraft[] = array(
                'location_icao' => 'LKPR', // Todo add location from $fleet['location_id']
                'fleet_id' => $fleet['id'],
                'name' => $fleet['name'],
        ...
        return response(json_encode($aircraft),200);
    ...
    public function pilotConnection(Request $request)
    {
        ...
        $reservation = Flight::where('user_id', $pilotid)->with('user')->with('airline')->with('depapt')->with('arrapt')->with('aircraft')->first();
    ...
    public function liveAcars(Request $request)
    {
        ...
        $rpt = ACARSData::firstOrNew(['flight_id' => $reservation->id, 'user_id' => $pilotid]);
        $rpt->user()->associate($pilotid);
        $rpt->flight()->associate($reservation->id);
    ...
    // Todo: Ask Taylor how to fix this function.
    public function receiveVamPirep(Request $request)
    {
        ...
        $bid = Flight::where('user_id', $pilotID)
            ->with([
                'airline',
        ...
        $pirepExist = Flight::where('schedule_data', $flightId)->first();
        if($pirepExist) {
    ...
    public function deleteData(Request $request)
    {
        ...
        $pirep = Flight::where('schedule_data',$flightID)->first();
        if ($pirep) {
            $pirepId = $pirep->id;
